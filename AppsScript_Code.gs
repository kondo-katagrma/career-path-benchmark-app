...
function buildFullPrompt(lawName, target, base, deck1Text, deck2Text, respondentText){
  ...
  L.push('- 8. 提案：キャリアパス策定の方向性（具体的・実行可能な提案。第1回の決裁権者の意向、第2回のミドルリーダーの発言と直接紐づけて書く）');
  L.push('');
  ...
  L.push('以下は、同じ形式で過去に作成した完成レポートの抜粋です（別法人・匿名化済み）。個人名や具体的な数値は今回の対象法人のものに置き換えつつ、この粒度・構成・トーンを目標としてください。');
  L.push('');
  L.push('> 総括');
  L.push('> 貴法人が第1回で掲げた方向性は、職員の実態・志向と高い水準で一致しています。キャリアパス策定は「新たに課題を正す」というより、すでに芽生えている強みと職員の前向きな成長意欲を、仕組みとして定着させるフェーズにあります。特に次の3点が明確です。');
  L.push('> 1. 職員の課題意識は「自分がどう成長するか」に強く向いており、自分事化が進んでいる。改善点の記述の大半が他責でなく自己成長への言及で、育成の土台となる文化があります。');
  L.push('> 2. 「〜のスペシャリストになりたい」という専門職志向が突出している。第1回で構想された「管理職コースと専門職・分野別リーダーコースの複線設計」は、職員の志向と強く合致します。');
  L.push('> 3. 副主任の役割の重さと、サポーター層の裁量の曖昧さが、職務・職責を定義すべき最優先ポイントとして浮かんでいる。これは第2回ヒアリングの内容とも重なります。');
  L.push('>');
  L.push('> 3. やりがいと改善点（課題意識）');
  L.push('> 改善点（＝課題意識）：記述のほぼ全員が「自分がどう成長・改善するか」を語っており、自分事化が非常に進んでいます（法人への不満ではなく、自らの伝え方・判断力・全体把握力を高めたいという内容）。そのうえで、構造的な課題として次が浮かびます。');
  L.push('> ・副主任の役割の重さと定義の曖昧さ：「副主任とクラス担任の両立が難しく、園全体に目を向ける余裕が持てない」。複数施設の副主任が同じ趣旨を述べています。');
  L.push('> ・サポーター層の裁量・役割の不明確さ：「サポーターとしてどこまで判断してよいか迷う」。');
  L.push('');
  L.push('この抜粋のように、①自由記述から実際の言い回しを（個人が特定されない範囲で）引用する、②同趣旨の記述をまとめて「複数の〜が同じ趣旨を述べています」と明記する、③各項目の最後にキャリアパス設計への含意を書く、という書き方を踏襲してください。');
  L.push('');
  L.push('## トーン規則（必守）');
