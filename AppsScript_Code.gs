...
function buildFullPrompt(lawName, target, base, deck1Text, deck2Text, respondentText){
  ...
  var L=[];
  L.push('# 依頼：キャリアパス策定支援における「現状把握アンケート分析」レポートを、1つの完成文書として作成してください。');
  L.push('対象法人：'+(lawName||'（法人名）'));
  L.push('');
  L.push('以下の3種類の資料（①アンケートの匿名化された自由記述・集計値、②全法人ベースライン、③第1回・第2回打合せ資料の全文）をすべて読み込み、career-path-director-hearing / career-path-hearing-summary / career-path-survey-analysis の各Skillの方針に沿って、次の構成で「単一の統合レポート」を作成してください。方針メモやヒアリング要点整理を独立した別文書として出力せず、その内容は該当箇所（特に総括と8.提案）に溶け込ませてください。');
  L.push('');
  L.push('## レポート構成（1つの文書として。見出しは以下の順番・粒度で）');
  L.push('- 総括（2〜4点。①アンケート・②第1回資料（決裁権者の意向）・③第2回資料（ミドルリーダーの声）の3つを突き合わせた、最も重要な構造的発見。各項目に根拠を明記）');
  L.push('- 1. 属性と職務内容・役割の傾向（役職別・施設別の違いも明示。表形式で提示）');
  L.push('- 2. 勤続年数と経験年数（役職別。表形式で提示。中途比率や、全法人ベースラインと比べた勤続・経験年数の傾向から読み取れる含意も書く。下記「勤続年数・経験年数・中途比率の解釈について」を参照）');
  L.push('- 3. やりがいと改善点（課題意識）');
  L.push('- 4. 取得したいスキル・学びたいテーマ');
  L.push('- 5. キャリア目標の傾向とサポートの必要性');
  L.push('- 6. 研修・学びの機会に対する満足度');
  L.push('- 7. 法人・上司へのキャリアに関する要望');
  L.push('- 8. 提案：キャリアパス策定の方向性（具体的・実行可能な提案。第1回の決裁権者の意向、第2回のミドルリーダーの発言と直接紐づけて書く。専門職・分野別リーダー等の複線コースを提案する場合は、下記「複線設計の提案について」の但し書きを必ず添えること）');
  L.push('- 9. その他（キャリアパスに直接関係しないが、2件以上の回答者から挙がった要望・気になる点を、集計セクションとは別に最後にまとめる）');
  L.push('');
  ...
  L.push('- 第2回ヒアリングの発言の中に、法人・決裁権者・園長への直接的な批判に類する内容が含まれる場合は、その発言をそのまま本文に引用しないこと。内容を一般化・構造化したうえで扱うか、本文には含めずレポート末尾に「次回打合せでの確認事項」として簡潔に触れるにとどめる。');
  L.push('- ただし、読み手の心証を気にして事実や重要な論点そのものを省略しすぎないこと。事実・数値は正確に書き、配慮するのは表現の仕方（言い回し）のみとする。');
  L.push('- 「①アンケート」「②第1回資料」「③第2回資料」という資料の呼び方の中では「決裁権者」という言葉を使ってよいが、レポート本文（総括や各セクションの文章）の中で読み手を「決裁権者は〜」のように名指しする表現は使わないこと。読み手＝決裁権者そのものであるため、本文では理事長・園長など実際の役職名で書くか、「貴法人」を主語にすること。');
  L.push('');
  ...
  L.push('- 設問の設計によって生まれる見かけ上の傾向を、組織の成果や特徴であるかのように書かないこと。例えば「改善が必要な点」という設問は回答者本人の内省を促す設計になっているため、回答が自己成長への言及に集中するのは設問上自然な結果であり、それ自体を「自分事化が進んでいる」のようなポジティブな組織的特徴として強調しないこと。');
  ...
  L.push('');
  L.push('## 勤続年数・経験年数・中途比率の解釈について（2.のセクションで必ず触れる）');
  L.push('- 中途（経験＞勤続）の比率が全法人ベースラインより高い場合：即戦力を外部採用で補っている状態であり、法人の理念・保育観の浸透に時間がかかりやすい／浸透度合いにばらつきが出やすいという含意がある。中途の割合が高いこと自体を否定せず、「理念共有の仕組み（研修・OJT・面談等）が追いついているか」という観点を、8.提案につなげる形で指摘すること。');
  L.push('- 全法人ベースラインと比べて勤続年数・経験年数のいずれも低い傾向にある場合：組織全体の経験の蓄積が薄く、指導・育成を担える層が手薄になりやすい／特定の少数のベテランに指導が集中しやすいという含意がある。逆にいずれも高い場合は、組織としての安定・定着は強みである一方、次の世代への権限移譲や新しい発想の取り込みが課題になりやすい。');
  L.push('- これらは一般論であり、実際にどちらの含意が当てはまるかは、①-3の自由記述（特に役職者の記述）や②③の資料と突き合わせて確認し、根拠のある形で書くこと（一般論をそのまま断定的に書かない）。');
  L.push('');
  L.push('## 見やすさの工夫（重要）');
  ...
  L.push('>');
  L.push('> 3. やりがいと改善点（課題意識）');
  L.push('> 改善点の自由記述は、設問の性質上「自分は何を改善したいか」という内省的な語りが中心になりやすい点をふまえたうえで読み解くと、次の構造的な課題が浮かびます。');
